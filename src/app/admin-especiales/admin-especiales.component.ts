...
@Component({
  selector: 'app-admin-especiales',
  templateUrl: './admin-especiales.component.html',
  styleUrls: ['./admin-especiales.component.css']
})
export class AdminEspecialesComponent {
  ...
  ngOnInit(): void {
   ...
   update(tableData: any): void {
     ...
     this.apiService.actualizarEspecial(id, formData).subscribe(
       response => {
   ...
   sendata(formulario:FormGroup){
     ...
     //   this.getdata();
     // })
     this.apiService.registerEspecial(formulario.value).subscribe((data: any) => {
       console.log(data)
     })
   }
   ...
   delete(index:number, tableData:any){
     ...
     this.apiService.borrarEspecial(this.formulario.id).subscribe(
       response => {
