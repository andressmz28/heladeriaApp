...
@Component({
  selector: 'app-admin-salsas',
  templateUrl: './admin-salsas.component.html',
  styleUrls: ['./admin-salsas.component.css']
})
export class AdminSalsasComponent {
  ...
  ngOnInit(): void {
   ...
   update(tableData: any): void {
     ...
     this.apiService.actualizarSalsa(id, formData).subscribe(
       response => {
   ...
   sendata(formulario:FormGroup){
     ...
     //   this.getdata();
     // })
     this.apiService.registerSalsa(formulario.value).subscribe((data: any) => {
       console.log(data)
     })
   }
   ...
   delete(index:number, tableData:any){
     ...
     this.apiService.borrarSalsa(this.formulario.id).subscribe(
       response => {
