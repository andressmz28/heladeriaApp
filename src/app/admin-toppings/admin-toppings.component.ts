...
@Component({
  ...
  styleUrls: ['./admin-toppings.component.css']
})
export class AdminToppingsComponent {
  ...
  ngOnInit(): void {
   ...
   update(tableData: any): void {
     ...
     this.apiService.actualizarTopping(id, formData).subscribe(
       response => {
   ...
   sendata(formulario:FormGroup){
     ...
     //   this.getdata();
     // })
     this.apiService.registerTopping(formulario.value).subscribe((data: any) => {
       console.log(data)
     })
   }
   ...
   delete(index:number, tableData:any){
     ...
     this.apiService.borrarTopping(this.formulario.id).subscribe(
       response => {
